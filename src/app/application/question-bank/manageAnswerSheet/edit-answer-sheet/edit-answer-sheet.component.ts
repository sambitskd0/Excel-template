...
@Component({
  selector: "app-edit-answer-sheet",
  templateUrl: "./edit-answer-sheet.component.html",
  styleUrls: ["./edit-answer-sheet.component.css"],
})
export class EditAnswerSheetComponent implements OnInit {
  ...
  constructor(
    ...
    this.answerSheetId = this.activatedRoute.snapshot.paramMap?.get("encId");
  }
  ...
  getAnswerSheetDetails() {
    this.questionBankService
      .getAnswerSheetDetails(this.answerSheetId)
      .subscribe({
        next: (response: any) => {
          if (response?.success) {
            this.answerSheetDetails = response?.data;
            this.fillTheForm();
          } else {
  ...
  fillTheForm() {
    this.selectedAssesmentType = this.answerSheetDetails.assessmentType;
    this.selectedClass = this.answerSheetDetails.classId;
    this.selectedStream = this.answerSheetDetails.streamType;
    this.selectedGroup = this.answerSheetDetails.streamGroupType;
    this.selectedSection = this.answerSheetDetails.section;

    this.selectedsetName = this.answerSheetDetails.setId;
    this.answerSheetFile = this.answerSheetDetails.answerSheetFile;
    this.uloadedFile =
      this.fileUrl + this.answerSheetDetails.answerSheetFile.replace(".", "~");

    this.totalMark = this.answerSheetDetails.totalMark;
    this.markObtained = this.answerSheetDetails.obtainedMark;

    this.dateOfExam = new Date(this.answerSheetDetails.dateOfExam);
    this.getStudents();
    ...
    this.commonserviceService
      ...
      .subscribe({
        next: (res: any) => {
          ...
          this.streamGroupAnnexture =
            this.annextureData?.STREAM_GROUP_TYPE.filter(
  ...
  getSubjects() {
    ...
    if (this.selectedClass > 10) {
      ...
      if (this.isScienceStreamSelected) this.getStreamGroupType();
    } else {
      this.selectedStream = "";
      this.selectedGroup = "";
      ...
      .subscribe({
        next: (response: any) => {
          if (response?.success === true) {
            ...
            this.selectedSubject = this.answerSheetDetails.subjectId;
          }
  ...
  getStudents() {
    ...
    const isClassSectionSelected =
      parseInt(this.selectedSection) > 0 && parseInt(this.selectedClass) > 0;
      ...
      this.questionBankService
        ...
        .subscribe({
          next: (response: any) => {
            ...
            this.selectedStudent = this.answerSheetDetails.studentId;
          },
        });
    }
  }
  ...
  getTotalMark() {
    const validationArr = [
      parseInt(this.selectedAssesmentType) > 0,
      parseInt(this.selectedClass) > 0,
      parseInt(this.selectedSubject) > 0,
      parseInt(this.selectedsetName) > 0,

      parseInt(this.selectedClass) > 10
        ? parseInt(this.selectedStream) > 0
        : true,
      parseInt(this.selectedClass) > 10 && parseInt(this.selectedStream) === 3
        ? parseInt(this.selectedGroup) > 0
        : true,
    ...
    if (!validationArr.includes(false)) {
      const params = {
        assessmentType: parseInt(this.selectedAssesmentType),
        classId: parseInt(this.selectedClass),
        subjectId: parseInt(this.selectedSubject),
        streamId: parseInt(this.selectedStream),
        groupId: parseInt(this.selectedGroup),
        setId: parseInt(this.selectedsetName),
        schoolId: this.userProfile?.school,
      };
  ...
  submitAnswerSheet() {
    ...
    this.questionBankService.submitAnswerSheet(this.formData).subscribe({
      next: (res: any) => {
        ...
        if (res.success === true) {
          this.alertHelper
            .viewAlert("success", "Uploaded", res?.msg)
            .then((res: any) => {
  ...
  prepareFormData() {
    ...
    this.formData.set("answerSheetId", this.answerSheetId);
