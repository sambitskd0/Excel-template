...
@Component({
  selector: 'app-caste-wise-enrollment-report',
  templateUrl: './caste-wise-enrollment-report.component.html',
  styleUrls: ['./caste-wise-enrollment-report.component.css']
})
export class CasteWiseEnrollmentReportComponent implements OnInit {
  ...
  loadData(params: any){
    ...
    this.isLoading = true;
    this.studentMisService.getCasteWiseReport(paramObj).subscribe({
      next: (res: any) => {
        this.resultListData = res?.data; // merge with existing data
        this.allStuTotal = res?.allStuTotal;
        this.casteType = res?.casteType;
