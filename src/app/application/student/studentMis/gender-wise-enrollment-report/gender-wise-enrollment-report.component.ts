...
@Component({
  selector: 'app-gender-wise-enrollment-report',
  templateUrl: './gender-wise-enrollment-report.component.html',
  styleUrls: ['./gender-wise-enrollment-report.component.css']
})
export class GenderWiseEnrollmentReportComponent implements OnInit {
  ...
  loadData(params: any){
    ...
    this.isLoading = true;
    this.studentMisService.getGenderWiseReport(paramObj).subscribe({
      next: (res: any) => {
        this.resultListData = res?.data; // merge with existing data
        this.minClass = res?.minClass;
