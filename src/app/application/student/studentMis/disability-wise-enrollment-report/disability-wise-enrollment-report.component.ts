...
@Component({
  selector: 'app-disability-wise-enrollment-report',
  templateUrl: './disability-wise-enrollment-report.component.html',
  styleUrls: ['./disability-wise-enrollment-report.component.css']
})
export class DisabilityWiseEnrollmentReportComponent implements OnInit {
  ...
  loadData(params: any){
    ...
    this.isLoading = true;
    this.studentMisService.getDisabilityWiseReport(paramObj).subscribe({
      next: (res: any) => {
        this.resultListData = res?.data; // merge with existing data
        this.allStuTotal = res?.allStuTotal;
        this.disabilityType = res?.disabilityType;
        this.countDisabLenght = res?.countDisabLenght;
