 ...
 * Description : Add Student Information (Registration)
 **/
...
export class AddStudentComponent implements OnInit {
  ...
  reset() {
    // It will clear the value of the input
    ...
    this.casteCertificatePhoto.nativeElement.value = "";
  }
  ...
  initializeForm() {
    this.studentForm = this.formBuilder.group({
      ...
      incomeLimit: [this.incomeLimit],            incomeCertificate: [this.incomeCertificate],
  ...
  setFormData() {
    ...
    // // this.formData.append("medicine", this.studentForm.get("medicine")?.value);
  ...
  fileUploadHandler(event: any, controlName: string) {
    ...
    if (uploadedFile != null) {
      ...
      if (controlName == 'studentPhotoID') {
        if (uploadedFile.type != "image/png" && uploadedFile.type != "image/jpg" && uploadedFile.type != "image/jpeg") {
          event.target.value = "";
        ...
        if (uploadedFile.type != "image/png" && uploadedFile.type != "image/jpg" && uploadedFile.type != "image/jpeg" && uploadedFile.type != "application/pdf") {
          event.target.value = "";
          this.resetFileControls(controlName);
          this.studentForm.patchValue({ question: "" });
          this.alertHelper.viewAlertHtml(
  ...
  resetFileControls(controlName: string) {
    // console.log('inside resetFileControls');
    this.studentForm.controls[controlName].patchValue("");
    switch (controlName) {
      // case "class":
      //   this.studentForm.patchValue({ class: "" });
      //   break;
      //   case "districtId":
      //   this.studentForm.patchValue({ districtId: "" });
      //   break;
      //   case "blockId":
      //   this.studentForm.patchValue({ blockId: "" });
      //   break;
      //   case "clusterId":
      //   this.studentForm.patchValue({ clusterId: "" });
      //   break;
      //   case "bankName":
      //   this.studentForm.patchValue({ bankName: "" });
      //   break;
      case "studentPhotoID":
  ...
  onReset() {
    ...
    this.studentForm.patchValue({ class: "" });
    // this.studentForm.controls['class'].patchValue('');
    // this.studentForm.setValue({class: "", bankName: ""});
    this.studentForm.patchValue({ districtId: "" });
    this.studentForm.patchValue({ blockId: "" });
    this.studentForm.patchValue({ clusterId: "" });
    this.studentForm.patchValue({ passedLastExam: "0" });
    // this.studentForm.patchValue({ gotMedicine: "0" });
    // this.studentForm.patchValue({ ironTablets: "0" });
    // this.studentForm.patchValue({ dewormingTablets: "0" });
    // this.studentForm.patchValue({ vitaminATablets: "0" });
    this.studentForm.patchValue({ bankName: "" });
  ...
  getMediumOfInstruction() {
    ...
    this.mediumOfInstruction.getmediumOfInstruction(this.schoolId).subscribe((res: any) => {
