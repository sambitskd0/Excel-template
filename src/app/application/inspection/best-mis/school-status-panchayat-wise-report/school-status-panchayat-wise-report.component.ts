...
@Component({
  selector: "app-school-status-panchayat-wise-report",
  templateUrl: "./school-status-panchayat-wise-report.component.html",
  styleUrls: ["./school-status-panchayat-wise-report.component.css"],
})
export class SchoolStatusPanchayatWiseReportComponent implements OnInit {
  ...
  loadDataRes(params: any) {
    ...
    this.isLoading = true;
    this.InspectionMis.schoolStatusPanchayatWiseReport(paramObj).subscribe({
      next: (res: any) => {
  ...
  excel(level: any) {
    ...
    this.InspectionMis.downloadSchoolStatusPanhayatWIseReportCsv(
      this.csvData
    ).subscribe((res: any) => {
      const data = res["data"];
      if (level == 0) {
      ...
      new ngxCsv(data, "openCloseSchoolReport", this.csvoptions);
      this.spinner.hide();
    });
  }
