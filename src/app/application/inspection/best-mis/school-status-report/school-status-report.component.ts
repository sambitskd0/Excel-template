...
@Component({
  selector: "app-school-status-report",
  templateUrl: "./school-status-report.component.html",
  styleUrls: ["./school-status-report.component.css"],
})
export class SchoolStatusReportComponent implements OnInit {
  ...
  loadDataRes(params: any) {
    ...
    this.isLoading = true;
    this.InspectionMis.openCloseReport(paramObj).subscribe({
      next: (res: any) => {
  ...
  excel(level: any) {
    ...
    this.InspectionMis.downloadOpenCloseCsv(this.csvData).subscribe(
      (res: any) => {
        const data = res["data"];
        //let title = "Header1 \r\n\n";
        if (level == 0) {
