...
@Component({
  selector: "app-school-status-nagar-wise-report",
  templateUrl: "./school-status-nagar-wise-report.component.html",
  styleUrls: ["./school-status-nagar-wise-report.component.css"],
})
export class SchoolStatusNagarWiseReportComponent implements OnInit {
  ...
  loadDataRes(params: any) {
    ...
    this.isLoading = true;
    this.InspectionMis.openCloseReportNagarWise(paramObj).subscribe({
      next: (res: any) => {
  ...
  excel(level: any) {
    ...
    this.InspectionMis.downloadOpenCloseNNCsv(this.csvData).subscribe(
      (res: any) => {
        const data = res["data"];
        if (level == 0) {
          this.csvoptions = {
