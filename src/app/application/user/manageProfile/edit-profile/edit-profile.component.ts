...
export class EditProfileComponent implements OnInit, AfterViewChecked {
	...
	blockData: any = [];
	offblockData: any = [];
	distLvl: boolean = true;
	blkLvl: boolean = true;
	clusterLvl: boolean = true;
	...
	locateId: any = "";
	txtName: string = "";
	radioGender: number = 0;
	dtDOB: any = "";
	vchImage: any = "";
	districtId: number = 0;
	blockId: number = 0;
	villageId: number = 0;
	nagarnigamId: number = 0;
	txtMobile: string = "";
	txtEmail: string = "";
	txtAddress: string = "";
	fileSource: any = "";
	...
	ngOnInit(): void {
		...
		if (this.userProfile.userLevel == 5) {
			...
			this.profileForm.get("userLevelId")?.patchValue(4);
	...
	editProfile(encId: string) {
		this.spinner.show();
		this.profileService.getProfile(encId).subscribe((res: any) => {
			this.profileData = res.data[0];
			this.encId = this.profileData.encId;

			this.txtName = this.profileData.vchfullName;
			this.radioGender = this.profileData.tinGenderId.toString();
			if (this.profileData.dtmDOB != null) {
				this.dtDOB = new Date(this.profileData?.dtmDOB.toString());
			} else {
				this.dtDOB = "";
			}

			this.vchImage = this.profileData.vchImage;
			if (
				this.profileData.vchImage !== "" &&
				this.profileData["vchImage"] !== null
			) {
				this.isimageUrlTeacher = true;
				var str = this.profileData.vchImage;
				var newstr = str.replace(".", "~");
				this.imageUrlTeacher = this.fileUrl + "/uploads/userProfiles/" + newstr;
				this.vchImage = this.profileData.vchImage;
				this.isimageUrlTeacherDb = true;
			}
			this.fileSource = this.profileData.fileSource;

			this.districtId = this.profileData.intContactDistId;
			this.blockId = this.profileData.intContactBlockId;
			// this.nagarnigamId = this.profileData.intContactPanchayatId;
			this.tagStatus = this.profileData.tagStatus;
			this.txtMobile = this.profileData.vchMobileNo;
			this.txtEmail = this.profileData.vchEmailId;
			this.txtAddress = this.profileData.vchContactAddress;
			this.offdistrictId = this.profileData.intOfficeDistId;
			this.offblockId = this.profileData.intOfficeBlockId;
			this.offclusterId = this.profileData.intOfficeClusterId;
			this.vchOfficePhoneNo = this.profileData.vchOfficePhoneNo;
			this.intDesignationId = this.profileData.intDesignationId;

			this.designationGroupId = this.profileData.designationGroupId;
			this.userLevelId = this.profileData.intLevelId;
			if (this.profileData?.locateId != null) {
				this.locateId = this.profileData?.locateId.toString();
			} else {
				this.locateId = 0;
			}
			...
			if (this.districtId > 0) {
				...
				this.blockId = this.profileData.intContactBlockId;
				// this.getPanchayat(this.districtId);
			}

			if (this.nagarnigamId > 0) {
				this.getVillage(this.nagarnigamId, this.locateId);
				this.villageId = this.profileData.intContactVillageId;
			}

			if (this.offdistrictId > 0) {
				this.getOffBlock(this.offdistrictId);
			}
			if (this.offblockId > 0) {
				this.getCluster(this.offblockId);
			}
			this.getSubDesignation(this.designationGroupId);
			// this.locateRadioControl(
			// 	this.offdistrictId,
			...
			if (this.tagStatus > 0) {
				this.profileForm.controls["userLevelId"].disable();
				this.profileForm.controls["designationGroupId"].disable();
				this.profileForm.controls["intDesignationId"].disable();
				this.profileForm.controls["offdistrictId"].disable();
				this.profileForm.controls["offblockId"].disable();
				this.profileForm.controls["offclusterId"].disable();
			}
			this.loadSubDesignation(this.userLevelId);
	...
	initializeForm() {
		this.profileForm = this.formBuilder.group({
			txtName: [this.txtName, [Validators.required, Validators.maxLength(64)]],
			radioGender: [this.radioGender, Validators.required],
			txtMobile: [
				this.txtMobile,
				[
			...
			offblockId: [this.offblockId],
			offclusterId: [this.offclusterId],
			vchOfficePhoneNo: [this.vchOfficePhoneNo],
			...
			txtAddress: [this.txtAddress, [Validators.maxLength(500)]],
	...
	onSubmit() {
		this.submitted = true;
		// this.customValidators.customFormValidationHandler(this.profileForm);
		...
		this.alertHelper.submitAlert().then((result: any) => {
			if (result.value) {
				...
				this.profileService.updateProfile(allValue).subscribe({
					next: (res: any) => {
	...
	handleFileInputTeacher(e: any) {
		...
		if (this.teacherImageChange == true) {
			this.profileForm.controls["vchImage"].setValidators([
				...
				// this.customValidators.fileSizeValidator(file, 200),
			]);
