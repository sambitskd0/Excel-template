...
@Component({
  selector: 'app-inventory-stock-status',
  templateUrl: './inventory-stock-status.component.html',
  styleUrls: ['./inventory-stock-status.component.css']
})
export class InventoryStockStatusComponent implements OnInit {
  ...
  getAnnextureData() {
    this.commonService
      .getCommonAnnexture(["BOOK_TYPE","DAMAGE_REASON","ASSET_TYPE", "INCENTIVE_UNIT"])
      .subscribe({
        next: (res: any) => {          
          this.spinner.hide();
  ...
  viewStockStatus(...params: any) {
    ...
    this.inventoryService.viewInventoryStockStatus(this.paramObj).subscribe({
      next: (res: any) => {        
 ...
 this.inventoryService.viewInventoryStockStatus(this.paramObj).subscribe({
  ...
  viewDamageStock(assetItemId:any){

    const obj ={assetItemId : assetItemId,schoolId:this.userProfile.school}
    this.inventoryService.viewDamageList(obj).subscribe({
      next: (res: any) => {
        this.damageListData = res?.data;
