...
@Component({
  selector: 'app-view-damage-book',
  templateUrl: './view-damage-book.component.html',
  styleUrls: ['./view-damage-book.component.css']
})
export class ViewDamageBookComponent implements OnInit,AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    if(this.userProfile.loginUserTypeId != 3){
      this.viewBookDamageData(this.getSearchParams());
    }else{
  ...
  getSearchParams() {
    return {
      ...
      searchBookType: this.searchBookType, 
      

    };
  }
  viewBookDamageData(...params: any) {
    this.spinner.show();
    const {
      previousSize,
      offset,
    ...
    //   .set("searchAcademicYear", searchAcademicYear)
    //   .set("searchBookType", searchBookType);       // set query params
    this.isLoading = true;

    this.librararyService.viewBookDamageData(this.paramObj).subscribe({
      next: (res: any) => {
  ...
  onDelete(encId: string) {   
    ...
    .then((result) => {
      if (result.value) {
        ...
        this.spinner.show();
        this.isLoading = true;
        this.librararyService.deleteDamageBook(paramList).subscribe({
          next: (res: any) => {
            if (res?.success === true) {
              this.alertHelper.successAlert(
                "Deleted!",
                "Damage book deleted successfully",
                "success"
              );
              this.viewBookDamageData(this.getSearchParams());
            } else {
