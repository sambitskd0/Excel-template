...
@Component({
  selector: 'app-view-book-receipt',
  templateUrl: './view-book-receipt.component.html',
  styleUrls: ['./view-book-receipt.component.css']
})
export class ViewBookReceiptComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if(this.userProfile.loginUserTypeId != 3){
      this.viewBookReceiptData(this.getSearchParams());
    }else{
  ...
  viewBookReceiptData(...params: any) {
    ...
    this.isLoading = true;
    this.spinner.show();
    this.librararyService.viewBookReceiptData(this.paramObj).subscribe({
      next: (res: any) => {
  ...
  onDelete(encId: string) {   
    ...
    .then((result) => {
      if (result.value) {
        ...
        this.spinner.show();
        this.isLoading = true;
        this.librararyService.deleteBookReceipt(paramList).subscribe({
          next: (res: any) => {
            if (res?.success === true) {
              this.alertHelper.successAlert(
                "Deleted!",
                "Book receipt deleted successfully",
                "success"
              );
              this.viewBookReceiptData(this.getSearchParams());
            } else {
  ...
  showAssetList(encId:string,schoolId:string){    
    this.spinner.show();
    this.librararyService.getBookReceiptList(encId,schoolId).subscribe({
      next: (resp: any) => {
