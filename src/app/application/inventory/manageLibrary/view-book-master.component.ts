...
import { RaiseGrievanceService } from '../../grievance/services/raise-grievance.service';
import { LibraryService } from '../services/library.service';


@Component({
  selector: 'app-view-book-master',
  templateUrl: './view-book-master.component.html',
  styleUrls: ['./view-book-master.component.css']
})
export class ViewBookMasterComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if(this.userProfile.loginUserTypeId != 3){
      this.viewBookMasterData(this.getSearchParams());
    }else{
  ...
  viewBookMasterData(...params: any) {
    ...
    this.librararyService.viewBookMasterData(this.paramObj).subscribe({
      next: (res: any) => {
  ...
  onDelete(encId: string) {   
    ...
    .then((result) => {
      if (result.value) {
        ...
        this.spinner.show();
        this.isLoading = true;
        this.librararyService.deleteBookMasterData(paramList).subscribe({
          next: (res: any) => {
            if (res?.success === true) {
              this.alertHelper.successAlert(
                "Deleted!",
