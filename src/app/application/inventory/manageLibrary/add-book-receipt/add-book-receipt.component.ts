...
@Component({
  selector: 'app-add-book-receipt',
  templateUrl: './add-book-receipt.component.html',
  styleUrls: ['./add-book-receipt.component.css']
})
export class AddBookReceiptComponent implements OnInit,AfterViewInit {
  openingStock!: any;
  ...
  getBookList() {
    ...
    this.spinner.show(); //getBookMaster
    this.librararyService.getBookAcrdToStock('',this.userProfile.school).subscribe({
      next: (data: any) => {
   ...
   bookReceiptInfo() {
    return this.openingStock.get("bookReceiptArray") as FormArray;
  }
  filterBookName(id: any, index: number) {
    
    let levelControl = <FormArray>this.openingStock.controls['bookReceiptArray'];
    if (id !== "") {
  ...
  initializeForm() {
    this.openingStock = this.formBuilder.group({
      ...
      receivedFrom: [this.receivedFrom,[Validators.maxLength(50),Validators.pattern(/^[a-zA-Z0-9.',-/ ]*$/),this.customValidators.firstCharValidatorRF]],
      billNo: [this.billNo,[Validators.maxLength(50),Validators.pattern(/^[a-zA-Z0-9.,-/ ]*$/),this.customValidators.firstCharValidatorRF]],
      recvDate: [this.recvDate,[Validators.required]],
      createdBy: [this.userProfile.userId],
      schoolId: [this.userProfile.school],
      bookReceiptArray: this.formBuilder.array([]), // store all data in this array
   ...
   newOpeningStock() {
    return this.formBuilder.group({
      ...
      bookNo: [this.bookNo, [Validators.required,Validators.maxLength(20),Validators.pattern(/^[a-zA-Z0-9.,-/ ]*$/),this.customValidators.firstCharValidatorRF]],      
      amount: [this.amount, [Validators.required,Validators.maxLength(20),Validators.pattern(/^[1-9]\d*(\.\d+)?$/),this.customValidators.firstCharValidatorRF]],      
    });
  }
  addRow(index: any) {
    let emptyRow: Boolean = false;
    this.bookReceiptInfo()?.controls?.map((item: any, index: number) => {
    ...
    if (emptyRow === false) {
      this.bookReceiptInfo().insert(index + 1, this.newOpeningStock());
    }
  }
  // remove row
  removeRow(index: any) {
    this.bookFilterData.splice(index, 1);
    if(this.bookReceiptInfo().length === 1){
      this.resetForm();
    }
    this.bookReceiptInfo().length > 1 &&
      this.bookReceiptInfo().removeAt(index);
    this.checkSingleField();
  }
  checkSingleField() {
    this.singleField = this.bookReceiptInfo()?.length > 1 ? false : true;
  }
  resetFormArray() {
    (this.openingStock.get("bookReceiptArray") as FormArray).clear();
  }
  resetForm() {
    this.resetFormArray();
  ...
  checkDuplicateLevel(): any {
    ...
    let formLablesArr = <FormArray>(
      this.openingStock.controls["bookReceiptArray"]
    );
  ...
  validateStockForm() {
    let allErrors: any = [];
    let authorityLevelsArr = <FormArray>this.openingStock.controls["bookReceiptArray"];
  ...
  submitStock(){
    ...
    if (this.openingStock.valid == true) {
      this.alertHelper.submitAlert().then((result) => {
        if (result.value) {
          this.spinner.show();
          this.librararyService.addBookReceipt(this.openingStock.value).subscribe({
            next: (res: any) => {
              this.spinner.hide();
              this.alertHelper
                .successAlert(
                  "Saved!",
                  "Book receipt created successfully.",
                  "success"
                )
                .then(() => {
                  this.initializeForm();
