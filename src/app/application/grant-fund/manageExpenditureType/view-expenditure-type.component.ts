...
@Component({
  selector: 'app-view-expenditure-type',
  templateUrl: './view-expenditure-type.component.html',
  styleUrls: ['./view-expenditure-type.component.css']
})
export class ViewExpenditureTypeComponent implements OnInit {
  public fileUrl = environment.filePath;
  viewExpenditureType!:FormGroup;
  select_all = false;
...
loadGrantData(...params: any){
  ...
  this.isLoading = true;
  this.manageExpendituretypeservice.viewExpenditureType(this.paramObj).subscribe({
    next: (res: any) => {
...
deleteExpenditureType(id: number) {
  ...
  this.userId = users?.userId;
  this.alertHelper
    .deleteAlert("Do you want to delete the selected expenditure type ?", "", "question", "Yes, delete it!")
    .then((result) => {
      if (result.value) {
        this.spinner.show();
        this.isLoading = true;
        this.manageExpendituretypeservice.deleteExpenditureType(id, this.userId, this.profileId)
          .subscribe({
            next: (res: any) => {
              if (res?.success === true) {
