...
@Component({
  selector: 'app-view-grant-info',
  templateUrl: './view-grant-info.component.html',
  styleUrls: ['./view-grant-info.component.css']
})
export class ViewGrantInfoComponent implements OnInit {
  ...
  GrantInfoData: any = [];
  dataSource = new MatTableDataSource(this.GrantInfoData);
  //end
  ...
  ngOnInit(): void {
    ...
    if(this.userProfile.loginUserTypeId == 2){
      this.schoolId = userProfile?.school;
      this.viewGrantInfo(this.getSearchParams());
    }else{
  ...
  this.isLoading = true;
  this.manageGrantInfoService.viewGrantInfo(this.paramObj).subscribe({
    next: (res: any) => {
      this.GrantInfoData.length = previousSize; // set current size
      this.GrantInfoData.push(...res?.data); // merge with existing data
      this.GrantInfoData.length = res?.totalRecord; // update length
      this.dataSource.paginator = this.paginator; // update paginator
      this.dataSource._updateChangeSubscription(); // update table
      this.isLoading = false;
      this.isNorecordFound = this.GrantInfoData.length ? false : true;
      this.spinner.hide();
    },
    error: (error: any) => {
      this.isLoading = false;
...
deleteGrantInfo(id: number,grantTypeId:any,schoolId:any) {
  const users = this.commonService.getUserProfile();
  this.userId = users?.userId;
  this.alertHelper
    .deleteAlert("Do you want to delete the selected grant info ?", "", "question", "Yes, delete it!")
    .then((result) => {
      if (result.value) {
        this.spinner.show();
        this.isLoading = true;
        this.manageGrantInfoService.deleteGrantInfo(id,this.userId,grantTypeId,schoolId, this.profileId)
          .subscribe({
            next: (res: any) => {
              if (res?.success === true) {
                this.alertHelper.successAlert(
                  "Deleted!",
                  "Grant info deleted successfully",
                  "success"
                );
                this.viewGrantInfo(this.getSearchParams());
              } else {
