...
@Component({
  selector: "app-view-grant-expenditure",
  templateUrl: "./view-grant-expenditure.component.html",
  styleUrls: ["./view-grant-expenditure.component.css"],
})
export class ViewGrantExpenditureComponent implements OnInit {
  paramObj: any;
  ...
  ngOnInit(): void {
    ...
    if (userProfile.loginUserTypeId == 2) {
      ...
      this.viewGrantExpenditure(this.getSearchParams());
      this.spinner.hide();
    } else {
  ...
  getSearchParams() {
    if(this.expenditureDate != ""){
      this.expenditureDateStr = this.commonFunctionHelper.formatDateHelper(this.expenditureDate);
    }
    return {
      ...
      expenditureDate: this.expenditureDateStr,
    };
  }
  ...
  viewGrantExpenditure(...params: any) {
    this.spinner.show();
    const {
      previousSize,
      offset,
    ...
    this.isLoading = true;
    this.manageGrantExpenditureService
      .viewGrantExpendeture(this.paramObj)
      .subscribe({
        next: (res: any) => {
    ...
    this.alertHelper
      ...
      .then((result) => {
        ...
        if (result.value) {
          ...
          this.isLoading = true;
          this.manageGrantExpenditureService
            .deleteGrantExpenduture(id, this.userId, this.profileId)
            .subscribe({
              next: (res: any) => {
                if (res?.success === true) {
                  this.alertHelper.successAlert(
                    "Deleted!",
                    "Expenditure detail deleted successfully",
                    "success"
                  );
                  this.viewGrantExpenditure(this.getSearchParams());
                } else {
