...
@Component({
  selector: 'app-view-change-request-application',
  templateUrl: './view-change-request-application.component.html',
  styleUrls: ['./view-change-request-application.component.css']
})
export class ViewChangeRequestApplicationComponent implements AfterViewInit, OnInit {
  ...
  takeActionApprovalForm!:FormGroup;
  takeActionBeoApprovalForm!:FormGroup;
  ...
  changeRequestApprovalAction: any = "";
  changeRequestApprovalRemark: any = "";
  beoApprovalAction: any = "";
  beoApprovalRemark: any = "";
  ...
  @ViewChild("closeApproveTakeAction") closeApproveTakeAction!: any;
  @ViewChild("closeBeoTakeAction") closeBeoTakeAction!: any;
  ...
  ngOnInit(): void {
    ...
    this.takeActionApprovalInitialzeForm();
    this.takeActionApprovalBeoInitialzeForm();
  ...
  takeActionApprovalInitialzeForm() {
    this.takeActionApprovalForm = this.formBuilder.group({
      teacherIdApprovalModal: [this.teacherencId],
      changeRequestApprovalAction: [this.changeRequestApprovalAction, [Validators.required]],
      changeRequestApprovalRemark: [
        this.changeRequestApprovalRemark,
        [Validators.required, Validators.maxLength(500)],
      ],
      updatedBy: [this.userProfile.userId],
      userTypeId: [this.loginUserType],
    });
  }
  takeActionApprovalBeoInitialzeForm() {
    this.takeActionBeoApprovalForm = this.formBuilder.group({
      teacherIdBeoModal: [this.teacherencId],
      beoApprovalAction: [this.beoApprovalAction, [Validators.required]],
      beoApprovalRemark: [
        this.beoApprovalRemark,
        [Validators.required, Validators.maxLength(500)],
      ],
      updatedBy: [this.userProfile.userId],
      userTypeId: [this.loginUserType],
  ...
  loadTeacherData(...params: any) {
    ...
    this.isLoading = true;
    this.teacherVerification.changeRequestApplications(paramObj).subscribe({
      next: (res: any) => {
  ...
  checkUncheckAll() {
    ...
    if (this.viewTableForm.get("checkAll")?.value !== true) {
      ...
      this.resultListData.forEach((eachdata: any) => {
        ...
        if((eachdata.changeRequestStatus == 1 && this.userDesignation == 46)){
  ...
  approveAllTeacher(){
    ...
    console.log(checkRecordArr.controls.length);
    ...
    if (this.approveAllForm.valid === true) {
      ...
      this.alertHelper.submitAlert().then((result: any) => {
        ...
        if (result.value) {
          this.spinner.show(); // ==== show spinner
          this.teacherVerification
            .changeRequestSubmit(this.approveAllForm.value)
            .subscribe({
              next: (res: any) => {
                this.spinner.hide(); //==== hide spinner
                this.alertHelper
  ...
  changeRequestApproveModal(encId: string, teacherName: any,teacherCode: any){
    this.teacherNameModal = teacherName;
    this.teacherencId = encId; //teacher Id
    this.teacherCodeModal = teacherCode; //teacher code
    this.takeActionApprovalInitialzeForm();
  }
  beoApproveModal(encId: string, teacherName: any,teacherCode: any){
    this.teacherNameModal = teacherName;
    this.teacherencId = encId; //teacher Id
    this.teacherCodeModal = teacherCode; //teacher code
    this.takeActionApprovalBeoInitialzeForm();
  }
  changeRequestApproveSubmit(){
    if ("INVALID" === this.takeActionApprovalForm.status) {
      for (const key of Object.keys(this.takeActionApprovalForm.controls)) {
        if (this.takeActionApprovalForm.controls[key].status === "INVALID") {
          const invalidControl = this.el.nativeElement.querySelector(
            '[formControlName="' + key + '"]'
          );
          invalidControl.focus();
          this.customValidators.formValidationHandler(
            this.takeActionApprovalForm,
            this.allLabel
          );
          break;
        }
    ...
    if (this.takeActionApprovalForm.invalid) {
      return;
    }
    if (this.takeActionApprovalForm.valid === true) {   
        
      this.alertHelper.submitAlert().then((result: any) => {
        if (result.value) {
          this.spinner.show(); // ==== show spinner
          this.teacherVerification
            .changeRequestApproveSubmit(this.takeActionApprovalForm.value)
            .subscribe({
              next: (res: any) => {
                this.spinner.hide(); //==== hide spinner
                this.alertHelper
                  ...
                  .then(() => {
                    this.closeApproveTakeAction.nativeElement.click();
                    this.loadTeacherData(this.getSearchParams());
                    this.takeActionApprovalInitialzeForm();
                  });
              },
  ...
  changeRequestBeoApprove(){
    if ("INVALID" === this.takeActionBeoApprovalForm.status) {
      for (const key of Object.keys(this.takeActionBeoApprovalForm.controls)) {
        if (this.takeActionBeoApprovalForm.controls[key].status === "INVALID") {
          const invalidControl = this.el.nativeElement.querySelector(
            '[formControlName="' + key + '"]'
          );
          invalidControl.focus();
          this.customValidators.formValidationHandler(
            this.takeActionBeoApprovalForm,
            this.allLabel
          );
          break;
        }
    ...
    if (this.takeActionBeoApprovalForm.invalid) {
      return;
    }
    if (this.takeActionBeoApprovalForm.valid === true) {  
         
      this.alertHelper.submitAlert().then((result: any) => {
        if (result.value) {
          this.spinner.show(); // ==== show spinner
          this.teacherVerification
            .changeRequestBeoApprove(this.takeActionBeoApprovalForm.value)
            .subscribe({
              next: (res: any) => {
                this.spinner.hide(); //==== hide spinner
                this.alertHelper
                  ...
                  .then(() => {
                    this.closeBeoTakeAction.nativeElement.click();
                    this.loadTeacherData(this.getSearchParams());
                    this.takeActionApprovalBeoInitialzeForm();
                  });
              },
