...
@Component({
  selector: 'app-view-joining-school',
  templateUrl: './view-joining-school.component.html',
  styleUrls: ['./view-joining-school.component.css']
})
export class ViewJoiningSchoolComponent implements OnInit {
    ...
    if (clusterId !== "") {
      this.commonService.getSchoolList(clusterId).subscribe((res: any) => {
        ...
        if (
          ...
          // this.getSchoolCategory(this.userProfile.schoolCategory);
        } else {
  ...
  //   this.scSchoolCatagorySelect = false;
  //   this.scSchoolCatagoryLoading = true;       

  //  this.schoolCatData = [];
    

  //   if (schoolCategoryId !== "") {
  //     this.transferService.getSchoolCategory(schoolCategoryId).subscribe((res: any) => {
  //       this.schoolCatData = res;
  //       this.schoolCatData = this.schoolCatData.data;

  //       if (this.userProfile.cluster != 0 || this.userProfile.cluster != "") {
  //         this.schoolCatData = this.schoolCatData.filter((clu: any) => {
  //           return clu.schoolCatagoryId == this.userProfile.cluster;
  //         });
  //              console.log(this.schoolCatData);     
  //       } else {
  //         this.scSchoolCatagorySelect = true;
  //       }
  //       this.scSchoolCatagoryLoading = false;
  //     });
  //   } else {
  //     this.scSchoolCatagorySelect = true;
  //     this.scSchoolCatagoryLoading = false;
  ...
  loadData(params: Object) {
    ...
    this.transferService.viewTeacherForJoining(params).subscribe({
      next: (response: any) => {
        ...
        this.isNorecordFound = this.filterData.teacherList.length
          ? false
          : true;
        this.spinner.hide();
      },
  ...
  showDetails(
    ...
    this.takeActionForm.patchValue({ joiningDate: this.joiningDate });
	}
  ...
  initializeTakeActionForm() {
		...
		this.takeActionForm = this.formBuilder.group({
			...
			// ],
      joiningDate: [(this.joiningDate) ? this.commonFunctionHelper.formatDateHelper(this.joiningDate):this.joiningDate],
  ...
  submitTakeAction() {
		...
		if (this.takeActionForm.controls["joiningDate"]?.value == "") {
			this.alertHelper.viewAlertHtml("error", "Invalid", "Joining Date is Required");
			return;
		}
		...
		if (this.takeActionForm.valid == true) {
			this.alertHelper.submitAlert().then((result: any) => {
				if (result.value) {
					this.spinner.show();
					this.transferService.updateJoiningDate(
						this.takeActionForm.getRawValue()
					).subscribe({
						next: (res: any) => {
							this.spinner.hide();
							this.alertHelper
								.successAlert("Saved!", "Joining date updated successfuly", "success")
								.then(() => {
