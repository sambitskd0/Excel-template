...
@Component({
  selector: 'app-nature-appointment-wise-teacher-report',
  templateUrl: './nature-appointment-wise-teacher-report.component.html',
  styleUrls: ['./nature-appointment-wise-teacher-report.component.css']
})
export class NatureAppointmentWiseTeacherReportComponent implements OnInit {
  ...
  loadData(params: any) {
    ...
    this.spinner.show();
    this.teacherMisService.loadNatureAppTypeWiseTeacherData(this.loadDataParam).subscribe({
      next: (res: any) => {
  ...
  execl(){
    ...
    this.teacherMisService.exportNatureAppTypeWiseTeacherReport(this.paramVal).subscribe({
      next: (res: any) => {
        let filepath = this.fileUrl1 + '/' + res.data.replace('.', '~');
        window.open(filepath);
        this.spinner.hide();
