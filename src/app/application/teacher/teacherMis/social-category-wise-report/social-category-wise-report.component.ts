...
@Component({
  selector: 'app-social-category-wise-report',
  templateUrl: './social-category-wise-report.component.html',
  styleUrls: ['./social-category-wise-report.component.css']
})
export class SocialCategoryWiseReportComponent implements OnInit {
  ...
  loadData(params: any) {
    ...
    this.spinner.show();
    this.teacherMisService.loadSocialCateWiseTeacherData(this.loadDataParam).subscribe({
      next: (res: any) => {
  ...
  execl(){
    ...
    this.teacherMisService.exportSocialCateWiseWiseTeacherReport(this.paramVal).subscribe({
      next: (res: any) => {
        let filepath = this.fileUrl1 + '/' + res.data.replace('.', '~');
        window.open(filepath);
        this.spinner.hide();
