...
@Component({
  selector: 'app-gender-wise-report',
  templateUrl: './gender-wise-report.component.html',
  styleUrls: ['./gender-wise-report.component.css']
})
export class GenderWiseReportComponent implements OnInit {
  ...
  getAnnextureDataBySeq() {
    ...
    this.commonService
      .getCommonAnnexture([
        "NATURE_OF_APPOINTMENT", "APPOINTMENT_TYPE", "SCHOOL_MANAGEMENT","GENDER"
      ], true)
      .subscribe({
        next: (res: any) => {
          this.spinner.hide();
  ...
  loadData(params: any) {
    ...
    this.spinner.show();
    this.teacherMisService.loadGenderWiseTeacherData(this.loadDataParam).subscribe({
      next: (res: any) => {
  ...
  execl(){
    ...
    this.teacherMisService.exportGenderWiseTeacherReport(this.paramVal).subscribe({
      next: (res: any) => {
        let filepath = this.fileUrl1 + '/' + res.data.replace('.', '~');
        window.open(filepath);
        this.spinner.hide();
