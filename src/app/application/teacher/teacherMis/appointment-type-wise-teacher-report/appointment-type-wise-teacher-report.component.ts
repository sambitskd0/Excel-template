...
@Component({
  selector: 'app-appointment-type-wise-teacher-report',
  templateUrl: './appointment-type-wise-teacher-report.component.html',
  styleUrls: ['./appointment-type-wise-teacher-report.component.css']
})
export class AppointmentTypeWiseTeacherReportComponent implements OnInit {
  ...
  loadData(params: any) {
    ...
    this.spinner.show();
    this.teacherMisService.loadAppTypeWiseTeacherData(this.loadDataParam).subscribe({
      next: (res: any) => {
  ...
  execl(){
    ...
    this.teacherMisService.exportAppTypeWiseTeacherReport(this.paramVal).subscribe({
      next: (res: any) => {
        let filepath = this.fileUrl1 + '/' + res.data.replace('.', '~');
        window.open(filepath);
        this.spinner.hide();
