...
@Component({
  selector: 'app-school-category-wise-teacher-report',
  templateUrl: './school-category-wise-teacher-report.component.html',
  styleUrls: ['./school-category-wise-teacher-report.component.css']
})
export class SchoolCategoryWiseTeacherReportComponent implements OnInit {
  ...
  loadData(param: any) {
    this.loadDataParam = {
      ...
      scClusterId: param.scClusterId,
      scSchoolId: param.scSchoolId,
      selManagement: param.selManagement,
      searchNatureOfAppointmt: param.searchNatureOfAppointmt,
      appointmtType: param.appointmtType,
      schoolCategory: this.userProfile.schoolCategory,
      serviceType: "report"
    }
    ...
    this.teacherMisService.loadSchoolCatWiseTeacherData(this.loadDataParam).subscribe({
      next: (res: any) => {
  ...
  drillDown(scDistrictId: any, scBlockId: any, scClusterId: any, scSchoolId: any, selManagement: any, searchNatureOfAppointmt: any, appointmtType: any) {
    this.paramDrillDown = {
    ...
    console.log(this.paramDrillDown);
    if (scDistrictId != '') {
  ...
  execl(){
    ...
    this.teacherMisService.exportSchoolCatWiseTeacherReport(this.paramVal).subscribe({
      next: (res: any) => {
        let filepath = this.fileUrl1 + '/' + res.data.replace('.', '~');
        window.open(filepath);
        this.spinner.hide();
