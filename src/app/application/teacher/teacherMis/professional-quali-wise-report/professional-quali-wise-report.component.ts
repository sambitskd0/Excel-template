...
@Component({
  selector: 'app-professional-quali-wise-report',
  templateUrl: './professional-quali-wise-report.component.html',
  styleUrls: ['./professional-quali-wise-report.component.css']
})
export class ProfessionalQualiWiseReportComponent implements OnInit {
  ...
  getAnnextureDataBySeq() {
    ...
    this.commonService
      .getCommonAnnexture([
        "NATURE_OF_APPOINTMENT", "APPOINTMENT_TYPE", "SCHOOL_MANAGEMENT","TCHR_PROFESSIONAL_QUALIFICATION"
      ], true)
      .subscribe({
        next: (res: any) => {
          this.spinner.hide();
          ...
          this.teacherEduData = res?.data?.TCHR_PROFESSIONAL_QUALIFICATION;
  ...
  loadData(params: any) {
    ...
    this.spinner.show();
    this.teacherMisService.loadProfessionalWiseTeacherData(this.loadDataParam).subscribe({
      next: (res: any) => {
  ...
  execl(){
    ...
    this.teacherMisService.exportProfessionalWiseTeacherReport(this.paramVal).subscribe({
      next: (res: any) => {
        let filepath = this.fileUrl1 + '/' + res.data.replace('.', '~');
        window.open(filepath);
        this.spinner.hide();
