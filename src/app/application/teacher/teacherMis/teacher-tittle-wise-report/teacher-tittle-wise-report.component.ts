...
@Component({
  selector: 'app-teacher-tittle-wise-report',
  templateUrl: './teacher-tittle-wise-report.component.html',
  styleUrls: ['./teacher-tittle-wise-report.component.css']
})
export class TeacherTittleWiseReportComponent implements OnInit {
  ...
  getAnnextureDataBySeq() {
    ...
    this.commonService
      .getCommonAnnexture([
        "NATURE_OF_APPOINTMENT", "APPOINTMENT_TYPE", "SCHOOL_MANAGEMENT","TEACHER_TITLE"
      ], true)
      .subscribe({
        next: (res: any) => {
          this.spinner.hide();
          this.teacherNatureAppointment = res?.data?.NATURE_OF_APPOINTMENT;
  ...
  loadData(params: any) {
    ...
    this.spinner.show();
    this.teacherMisService.loadTeacherTitleWiseData(this.loadDataParam).subscribe({
      next: (res: any) => {
  ...
  execl(){
    ...
    this.teacherMisService.exportTeacherTitleWiseReport(this.paramVal).subscribe({
      next: (res: any) => {
        let filepath = this.fileUrl1 + '/' + res.data.replace('.', '~');
        window.open(filepath);
        this.spinner.hide();
