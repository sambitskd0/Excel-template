...
@Component({
	selector: 'app-view-transfer-request-block',
	templateUrl: './view-transfer-request-block.component.html',
	styleUrls: ['./view-transfer-request-block.component.css']
})
export class ViewTransferRequestBlockComponent implements OnInit {
	...
	allLabel: string[] = ["District","Block","Cluster","School Name", "Remark"];
	tId : any = '';
	cSchoolId : any = '';
	sclId:any='';
	constructor(
	...
	submitTakeAction() {
		...
		if (this.takeActionForm.valid == true){
			this.alertHelper.submitAlert().then((result: any) => {
				if (result.value) {
					this.spinner.show();
					this.transferService.transferTakeActionByBlock(
	...
	loadTeacherList(...params: any) {
		...
		const {
			...
			districtId,
			blockId,
			clusterId, searchSchoolId,searchTeacherTitle,searchNatureOfAppointment,
		} = params[0];
		...
		this.isLoading = true;
		this.transferService.teacherListForBlock(this.paramObj).subscribe({
			next: (res: any) => {
	...
	showDetails( tId: any,cSchoolId: any) {
		this.sclId= cSchoolId;
		this.takeActionForm.patchValue({ 
			tId: tId,
